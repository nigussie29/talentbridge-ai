...
"""
    ...
    st.caption(
        "This section updates automatically when you edit the résumé or change "
        "the target career in the sidebar."
    )

    if resume_text.strip() == "":
        st.info("Paste resume text or upload a resume PDF to see career readiness.")
    else:
        ...
        if len(detected_skills) == 0:
            ...
            career_recommendation = recommend_careers_from_resume(resume_text)

            st.subheader("Best-Fit Career Recommendations")
            st.success(
                "Best current fit: "
                f"{career_recommendation['recommended_career']} — "
                f"{career_recommendation['recommended_score']}%"
            )

            career_ranking_table = []
            for career_match in career_recommendation["rankings"]:
                gap_names = [
                    skill_display_names.get(skill, skill)
                    for skill in career_match["skill_gaps"]
                ]
                career_ranking_table.append(
                    {
                        "Rank": career_match["rank"],
                        "Career": career_match["career"],
                        "Fit Score": f"{career_match['fit_score']}%",
                        "Status": career_match["status"],
                        "Remaining Gaps": ", ".join(gap_names) or "None",
                    }
                )

            st.table(career_ranking_table)
            st.caption(
                "This ranking estimates current readiness from résumé evidence. "
                "It is guidance, not a hiring or eligibility decision."
            )
            ...
            st.subheader(f"Readiness for {target_career}")
            st.metric(
                f"{target_career} Readiness Score",
            ...
            for career_match in career_recommendation["rankings"]:
                resume_report_text += (
                    f"{career_match['rank']}. {career_match['career']}: "
                    f"{career_match['fit_score']}% "
                    f"({career_match['status']})\n"
                )
...
with tab5:
    ...
    st.write(
        "Compare one job description against multiple candidate resumes and rank candidates by readiness."
    )
    ...
    st.markdown(
        ...
        <div class="product-card">
            ...
            <p>
            Add the target role requirements. TalentBridge AI will compare each candidate
            against the required skills, semantic match, and skill confidence evidence.
            </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    st.markdown(
        """
        <div class="product-card">
            <h3>Step 2: Add Candidate Resumes</h3>
            <p>
            Paste up to three candidate resumes. The app will rank candidates and generate
            a recruiter-ready screening report.
            </p>
        </div>
        """,
        unsafe_allow_html=True
